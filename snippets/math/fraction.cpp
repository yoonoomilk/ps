...
public:
  ...
  friend ostream& operator<< (ostream& cout, const fraction& v) { return cout << v.n << '/' << v.d; }

  friend ll floor(const fraction& v) { return v.n / v.d - ((v.n % v.d != 0) && ((v.n < 0) != (v.d < 0))); }
  friend ll ceil(const fraction& v) { return v.n / v.d + ((v.n % v.d != 0) && ((v.n > 0) == (v.d > 0))); }
  friend ll round(const fraction& v) { return (v.n + v.d / 2 * ((v.n > 0) == (v.d > 0))) / v.d; }
  friend fraction abs(const fraction& v) { return fraction(abs(v.n), abs(v.d)); }
};